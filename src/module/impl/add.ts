...
export default function Add(database: Database, parameters: ModuleParameters, table: string): number | null {
    ...
    if (parameters.options.target && parameters.options.subKeys !== false) {
        ...
        let numericalValue = getSafe(currentValue, parameters.options.target) ?? 0

        if (isNaN(numericalValue)) {
            throw new Error(`Expect numerical value is NaN [Found "${numericalValue}" at ${table}/${parameters.id}]`)
        }

        parameters.data = setSafe(currentValue, parameters.options.target, numericalValue + parameters.data)
    } else {
        ...
        if (isNaN(currentValue.json)) {
            throw new Error(`Expect numerical value is NaN [Found "${currentValue}" at ${table}/${parameters.id}]`)
        }
